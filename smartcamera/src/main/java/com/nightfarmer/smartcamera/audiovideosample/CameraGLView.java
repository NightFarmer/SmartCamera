...
public final class CameraGLView extends GLSurfaceView {
    ...
    public void switchCamera() {
        ...
        mCameraHandler.switchCamera(cameraId);
    }
...
/*		// the frequency of refreshing of camera preview is at most 15 fps
        ...
        OrientationEventListener _orientationListener = new OrientationEventListener(context, SensorManager.SENSOR_DELAY_NORMAL) {

            @Override
            public void onOrientationChanged(int orientation) {
                CameraGLView.this.orientation = orientation;
//                Log.i("xxx", "" + orientation);
            }
        };
...
//                GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT | GLES20.GL_DEPTH_BUFFER_BIT);
                ...
                final Bitmap bmp = createBitmapFromGLSurface(0, 0, getWidth(), getHeight(), unused);
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        int angle = getAngleNeeded();
                        final Bitmap resultBitmap = rotaingImageView(angle, bmp);
                        final String filePath = saveBitmap(resultBitmap);
                        CameraGLView.this.post(new Runnable() {
                            @Override
                            public void run() {
                                if (showPicture) {
                                    hehe.setImageBitmap(bmp);
                                    hehe.setVisibility(VISIBLE);
                                    reset();
                                    if (cameraFinishCallback != null) {
                                        cameraFinishCallback.onFinish(filePath);
                                    }
                                }
                            }
                        });
